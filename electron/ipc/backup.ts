...
export function registerBackupHandlers() {
  ipcMain.handle("backup:exportJson", async () => {
    ...
    const result = await dialog.showSaveDialog(win ?? undefined!, {
      title: "Ekspor data Dompetku",
      defaultPath: `dompetku-backup-${new Date().toISOString().slice(0, 10)}.json`,
      filters: [{ name: "JSON", extensions: ["json"] }],
    });
    if (result.canceled || !result.filePath) return { ok: false };

    const sqlite = getRawSqlite();
    const dump: Record<string, unknown[]> = { _meta: [{ exportedAt: new Date().toISOString(), app: "dompetku", version: 1 }] as unknown[] };
  ...
  ipcMain.handle("backup:importJson", async () => {
    ...
    const result = await dialog.showOpenDialog(win ?? undefined!, {
      title: "Impor data Dompetku (akan menimpa data saat ini)",
      filters: [{ name: "JSON", extensions: ["json"] }],
      properties: ["openFile"],
    });
